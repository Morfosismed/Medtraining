        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="flex items-center justify-between mb-4">
              <h1 className="text-3xl font-display font-bold text-surgical-green">{topic?.title}</h1>
              <div className="flex bg-slate-100 p-1 rounded-xl border border-dark-border">
                <button 
                  onClick={() => setActiveTab('content')}
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'content' ? 'bg-medical-blue text-white shadow-lg' : 'text-slate-500 hover:text-surgical-green'}`}
                >
                  Contenido
                </button>
                <button 
                  onClick={() => setActiveTab('quiz')}
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'quiz' ? 'bg-medical-blue text-white shadow-lg' : 'text-slate-500 hover:text-surgical-green'}`}
                >
                  Quiz
                </button>
              </div>
            ...
            {activeTab === 'content' ? (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
                ...
                <Card className="border-none bg-slate-50">
                  <CardContent className="pt-6 prose prose-slate max-w-none">
                    <div className="markdown-body text-slate-700 leading-relaxed space-y-4">
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                {!quiz ? (
                  ...
                  <Card className="text-center p-12 bg-white border-dark-border shadow-xl">
                    <div className="w-20 h-20 rounded-full bg-medical-blue/10 flex items-center justify-center mx-auto mb-6">
                      <CheckCircle className="w-10 h-10 text-medical-blue" />
                    </div>
                    <CardTitle className="text-3xl mb-2 text-surgical-green">¡Quiz Completado!</CardTitle>
                    <p className="text-slate-500 mb-8">Has obtenido un puntaje de:</p>
                    <div className="text-6xl font-bold text-medical-blue mb-10">
                  ...
                  <Card className="border-dark-border bg-white shadow-lg">
                    <CardHeader>
                      ...
                      <CardTitle className="text-xl leading-relaxed text-surgical-green">{quiz.questions[currentQuestion].question_text}</CardTitle>
                    </CardHeader>
                        ...
                        <button
                          ...
                          className={`w-full p-5 rounded-2xl text-left border transition-all flex items-center justify-between group ${
                            selectedOption === idx 
                              ...
                              : 'border-dark-border bg-slate-50 text-slate-600 hover:border-slate-300 hover:bg-slate-100'
                          }`}
                        >
                          <span className="font-medium">{option.option_text}</span>
                          <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${
                            selectedOption === idx ? 'border-medical-blue bg-medical-blue' : 'border-slate-300'
                          }`}>
          ...
          <div className="lg:col-span-1">
            <Card className="h-[calc(100vh-160px)] flex flex-col border-dark-border bg-white shadow-2xl sticky top-24">
              <CardHeader className="border-b border-dark-border pb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <CardTitle className="text-base text-surgical-green">Asesor IA</CardTitle>
              ...
              <CardContent className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-hide">
                {messages.length === 0 && (
                  <div className="h-full flex flex-col items-center justify-center text-center p-6">
                    <div className="w-16 h-16 rounded-2xl bg-slate-50 flex items-center justify-center mb-4 border border-dark-border">
                  ...
                  <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[85%] p-3 rounded-2xl text-sm ${
                      msg.role === 'user' 
                        ...
                        : 'bg-slate-100 border border-dark-border text-slate-700 rounded-tl-none'
                    }`}>
                      {msg.text}
                    </div>
                  </div>
                ...
                {aiLoading && (
                  <div className="flex justify-start">
                    <div className="bg-slate-100 border border-dark-border p-3 rounded-2xl rounded-tl-none flex gap-1">
              ...
              <div className="p-4 border-t border-dark-border">
                <form onSubmit={handleAskAI} className="relative">
                  <Input 
                    ...
                    className="pr-12 bg-slate-50 border-dark-border focus:ring-medical-blue h-12 rounded-xl text-slate-900"
                    value={query}
